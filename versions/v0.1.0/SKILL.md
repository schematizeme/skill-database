...
metadata:
  version: 0.1.0
description: A skill de MODELAGEM DE BANCO DE DADOS da casa (schema design) — a disciplina normativa, agnóstica de linguagem, do **DESENHO do schema relacional em si**, antes de uma linha de código de aplicação. Rege a tradução do domínio em modelo relacional (entidades→tabelas, atributos→colunas, relações 1:1/1:N/N:N com tabela de junção), **normalização 1FN–3FN** (e quando **desnormalizar conscientemente**, com registro), **chaves** (o padrão da casa — **PK surrogate ULID/UUIDv7 interna** + a chave natural como `UNIQUE`; **identidade≠email**, como no IAM), **tipos corretos por coluna** (nada de "texto pra tudo"; dinheiro em inteiro/`numeric`, tempo em `timestamptz` UTC, enum como domínio), **constraints** (`NOT NULL`/default, `UNIQUE`, `CHECK`, `FOREIGN KEY` com `ON DELETE` **consciente**), **índices** (quando criar, compostos e a ordem das colunas, cobertura, parciais, sem redundância — e **PII nunca vira chave de índice/URL**), **migrations expand-contract reversíveis** (nunca `DROP`/rename destrutivo num passo — casa com a schematize-data), **particionamento** básico e o **piso de segurança/privacidade** (coluna PII marcada, base legal + retenção — LGPD). O comando `/database-design` projeta um schema a partir de uma descrição em linguagem natural do domínio e **emite o SQL (CREATE TABLE) + a migration expand-contract + um resumo do modelo** (pronto pro "database builder" do app consumir); `/database-review` revisa um schema existente (normalização, índices, FK, PII). Use SEMPRE que for **modelar/desenhar/revisar um schema relacional, uma tabela, as colunas, as chaves, os índices ou as FKs** — mesmo sem citar "banco" ou "modelagem", e mesmo que peça só "cria as tabelas pra isso". FRONTEIRA: a engenharia de **DADOS** (pipeline/ETL/ELT/CDC/streaming, dado como **contrato** versionado, event sourcing, qualidade/quarentena, lineage) é da **schematize-data**; a **IMPLEMENTAÇÃO** backend (queries **parametrizadas**, ORM/repos, rodar a migration, cache) é das **skills de linguagem** (go/rust/elixir/csharp/zig/ruby). Pareia com a **schematize-engineering** (a BASE: banco §10, dados/eventos, segurança/LGPD §13-15/§32, archive §28, DoD §35).
---
...
**Versão:** skill `schematize-database` v0.1.0. Changelog em `CHANGELOG.md`.
