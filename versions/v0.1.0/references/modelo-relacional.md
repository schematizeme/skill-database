...
create table item_pedido (
  id          bytea primary key,           -- ULID/UUIDv7 (ver §chaves)
  pedido_id   bytea not null references pedido(id) on delete cascade,
  produto_id  bytea not null references produto(id) on delete restrict,
...
create table perfil_usuario (
  usuario_id bytea primary key references usuario(id) on delete cascade,  -- PK = FK garante 1:1
  bio        text,
  avatar_url text
);
```
...
create table produto_categoria (
  produto_id   bytea not null references produto(id)   on delete cascade,
  categoria_id bytea not null references categoria(id) on delete cascade,
...
| **Identificador (PK/FK)** | `bytea`/`uuid` (ULID/UUIDv7) | Ver §chaves em `normalizacao-chaves-indices.md`. Nunca `text` livre. |
