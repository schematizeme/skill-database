#!/usr/bin/env bash
# install.sh — instala a skill schematize-database no projeto atual (Claude Code).
#
...
# Como funciona: copia o corpo da skill para .claude/skills/schematize-database/ e instala
# os comandos (já no padrão <slug>-*, globalmente únicos) ACHATADOS em
...
# Saída: .claude/skills/schematize-database/ e .claude/commands/<slug>-*.md no alvo.
set -euo pipefail

SKILL_NAME="schematize-database"
